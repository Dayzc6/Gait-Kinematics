...
新的正式主入口
- 采用已验证通过的 Vicon 接收流程
- 使用 ViconWorker -> SyncEngine -> WriterWorker 新架构
- IMU / Planter 允许缺失并优雅降级
- 提供简单命令行控制：start / stop / status / quit
"""
import os
import queue
import sys
...
class AppController:
    ...
    def print_status(self):
        ...
        planter_state = "connected" if self.planter_worker.is_connected() else "unavailable"
        latest = self.vicon_worker.get_latest_frame()
        print("-" * 60)
        print(f"Vicon: {vicon_state}")
        print(f"IMU: {imu_state}")
        print(f"Planter: {planter_state}")
