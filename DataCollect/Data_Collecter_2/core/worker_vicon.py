...
if __name__ == '__main__':
    ...
    print(f"[TEST] Segments: {segs}")
    print(f"[TEST] Markers: {markers}")
    
    # 创建接收线程
    worker = ViconWorker(config.VICON_HOST_IP, segs, markers)
    worker.start()
    
    # 测试获取数据
    time.sleep(2)
    for i in range(5):
        frame, seg_data, marker_data, occluded = worker.get_latest_frame()
        print(f"[TEST] 帧号: {frame}")
        if segs:
            print(f"[TEST] 第一个Segment: {segs[0]} -> {seg_data.get(segs[0])}")
        time.sleep(0.5)
