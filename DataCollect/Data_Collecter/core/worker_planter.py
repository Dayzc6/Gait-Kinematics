...
from config import PLANTER_BAUD_RATE, PLANTER_SENSOR_POINTS, PLANTER_PORT
...
class FootSensor():
    ...
    def __init__(self, port, is_left,ser=None,parent=None):
        ...
        self.data = [0] * PLANTER_SENSOR_POINTS
    ...
    def _parse_packet(self, packet: bytes, callback=None) -> bool:
        ...
        data_bytes_needed = 2 + 2 * PLANTER_SENSOR_POINTS
        ...
        while self.running.is_set():
            try:
                ...
                while True:
                    ...
                    # 3) 解析内容：18 × uint16（小端）
                    data_bytes = frame[2:2 + PLANTER_SENSOR_POINTS * 2]
                    if len(data_bytes) < PLANTER_SENSOR_POINTS * 2:
                        # 长度异常，跳过
                        continue
                    values = list(struct.unpack("<" + "H" * PLANTER_SENSOR_POINTS, data_bytes))
